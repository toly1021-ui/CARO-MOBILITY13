/* ═══════════════════════════════════════════════════════════
   CARO MOBILITY — 고객 앱 홈 리디자인 v3 (시안 레이아웃 그대로)
   ───────────────────────────────────────────────────────────
   · 히어로(다크+골드 CTA) / CARO THE BLACK 줄 / 예약확인·월렌트 타일 / 하단 탭바
   · 기존 onclick 기능 그대로 연결 (예약/블랙/내예약/월렌트)
   · 멱등(idempotent) — 재렌더에도 안전
   적용: index.html </body> 위, customer-monthly.js 줄 다음:
     <script src="customer-redesign.js?v=3"></script>
═══════════════════════════════════════════════════════════ */
(function(){
  'use strict';
  ...
  st.textContent=
    ...
    /* 히어로 */
    +'#home-screen .home-welcome-banner.caro-hero{position:relative;overflow:hidden;color:#fff;'
    +'background:linear-gradient(155deg,#262a33 0%,#15161b 70%);border-radius:24px;padding:26px 24px 24px;'
    +'box-shadow:0 18px 40px -22px rgba(20,22,28,.7);display:block;}'
    ...
    +'.caro-hero-h{font-size:1.9rem;font-weight:800;line-height:1.18;letter-spacing:-.02em;position:relative;z-index:1;color:#fff;}'
    ...
    +'background:linear-gradient(135deg,#20232b,#14151a);border:1px solid rgba(198,164,104,.3);'
    ...
    /* 하단 탭바 */
    +'#home-screen .home-body{padding-bottom:94px;}'
    +'.caro-tabbar{position:absolute;left:0;right:0;bottom:0;display:flex;justify-content:space-around;align-items:center;'
    +'padding:9px 12px calc(12px + var(--sab));background:rgba(243,245,249,.92);backdrop-filter:blur(16px);'
  ...
  function buildHero(){
    ...
    b.innerHTML=
      '<div class="caro-hero-glow"></div>'+CAR_ART
  ...
  function buildActions(){
    ...
    g.innerHTML=
      '<button class="caro-black-row" onclick="goBlackLabel()">'
        ...
        +'<span class="cb-arr"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M9 6l6 6-6 6"/></svg></span>'
      +'</button>'
  ...
  function boot(){
    ...
    if(home && window.MutationObserver){
      ...
      new MutationObserver(function(){
        ...
        requestAnimationFrame(function(){ pend=false; apply(); });
      }).observe(home,{childList:true,subtree:true,characterData:true});
    }
    console.log('[디자인] ✅ 홈 리디자인 v3 (시안 레이아웃 적용)');
  }
