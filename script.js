...
/* ===== 회원 탈퇴 모달 - 뒤로가기 버튼 추가 ===== */
(function addWithdrawBackButton(){
  'use strict';
  
  function addBackBtn(){
    var modal = document.getElementById('withdraw-confirm-modal');
    if(!modal) return false;
    
    var confirmBtn = document.getElementById('withdraw-confirm-btn');
    if(!confirmBtn) return false;
    
    if(modal.querySelector('.cw-back-btn')) return true;
    
    var backBtn = document.createElement('button');
    backBtn.className = 'cw-back-btn';
    ...
    console.log('✅ 회원 탈퇴 뒤로가기 버튼 추가됨');
    return true;
  }
  
  function hookOpenWithdrawal(){
    if(typeof window.openWithdrawal !== 'function') return false;
    if(window.openWithdrawal._backBtnHooked) return true;
    
    var orig = window.openWithdrawal;
    window.openWithdrawal = function(){
      orig.call(this);
      setTimeout(addBackBtn, 50);
      setTimeout(addBackBtn, 200);
    };
    window.openWithdrawal._backBtnHooked = true;
    console.log('🎯 openWithdrawal 가로채기 활성화');
    return true;
  }
  
  if(!hookOpenWithdrawal()){
    var tries = 0;
    var iv = setInterval(function(){
      tries++;
      if(hookOpenWithdrawal() || tries > 20) clearInterval(iv);
    }, 200);
  }
  
  setTimeout(addBackBtn, 500);
  setTimeout(addBackBtn, 1500);
  
  console.log('🎨 회원 탈퇴 뒤로가기 버튼 IIFE 활성화');
})();
